...
const OpportunityManagement = () => {
  ...
  useEffect(() => {
    const fetchOpportunities = async () => {
      ...
      try {
        const response = await fetch((import.meta.env.VITE_API_URL || "http://localhost:3000") + "/api/opportunities", {
          headers: {
  ...
  const handleDelete = async (_id) => {
    try {
      const token = localStorage.getItem("token"); 
      const response = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3000")}/api/opportunities/${_id}`, {
        method: "DELETE",
        headers: {
          Authorization: `Bearer ${token}`, 
          "Content-Type": "application/json",
