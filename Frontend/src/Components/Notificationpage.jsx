...
const NotificationsPage = () => {
  ...
  useEffect(() => {
    ...
    const socketIo = io((import.meta.env.VITE_API_URL || "http://localhost:3000") + "");  // Connect to the server
  ...
  const handleMarkAsRead = async (id, status, feedback) => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch(`${(import.meta.env.VITE_API_URL || "http://localhost:3000")}/api/notifications/${id}`, {
        method: 'PUT',
        headers: {
          Authorization: `Bearer ${token}`,
